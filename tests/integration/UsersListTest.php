...
class UsersListTest extends TestCase {
	...
	public function test_the_column_lands_after_role(): void {
		...
		$this->assertSame(
			array( 'cb', 'name', 'role', 'leanroles_tags', 'posts' ),
			array_keys( $columns )
		);
	}
	...
	public function test_the_column_is_appended_when_there_is_no_role_column(): void {
		...
		$this->assertSame( array( 'cb', 'name', 'leanroles_tags' ), array_keys( $columns ) );
	}
	...
	public function test_no_column_when_no_tags_exist(): void {
		...
		$this->assertArrayNotHasKey( 'leanroles_tags', $columns );
	}

	public function test_the_column_renders_a_badge_per_tag(): void {
		Store::add( $this->user_id, array( 'gold', 'wholesale' ) );

		$html = UsersList::render_column( '', 'leanroles_tags', $this->user_id );
	...
	public function test_views_gain_a_link_per_populated_tag(): void {
		...
		$this->assertArrayHasKey( 'leanroles_gold', $views );
		$this->assertStringContainsString( 'role=gold', $views['leanroles_gold'] );
		$this->assertStringContainsString( '(1)', $views['leanroles_gold'] );
	}
	...
	public function test_empty_tags_get_no_link(): void {
		...
		$this->assertArrayHasKey( 'leanroles_gold', $views );
		$this->assertArrayNotHasKey( 'leanroles_wholesale', $views );
	}
	...
	public function test_the_current_tag_is_marked(): void {
		...
		$this->assertStringContainsString( 'aria-current="page"', $views['leanroles_gold'] );
	}
	...
	public function test_bulk_add(): void {
		...
		$this->as_admin_request(
			array(
				'leanroles_bulk_add' => 'Add tag',
				'leanroles_bulk_tag' => 'gold',
				'users'              => $targets,
			),
			'leanroles_bulk_tags',
			'leanroles_bulk_nonce'
		);

		$location = $this->capture_redirect( array( UsersList::class, 'handle_bulk' ) );

		$this->assertStringContainsString( 'leanroles_bulk=added', $location );
		$this->assertStringContainsString( 'leanroles_count=3', $location );
	...
	public function test_bulk_remove(): void {
		...
		$this->as_admin_request(
			array(
				'leanroles_bulk_remove' => 'Remove tag',
				'leanroles_bulk_tag'    => 'gold',
				'users'                 => $targets,
			),
			'leanroles_bulk_tags',
		...
		$this->assertStringContainsString( 'leanroles_bulk=removed', $location );
	...
	public function test_bulk_does_nothing_without_a_tag(): void {
		...
		$this->assertStringContainsString( 'leanroles_bulk=nothing', $location );
	...
	public function test_bulk_is_a_no_op_when_no_button_was_pressed(): void {
		$this->as_admin_request( array( 'leanroles_bulk_tag' => 'gold' ) );
	...
	public function test_bulk_refuses_a_bad_nonce(): void {
		$this->as_admin_request(
			array(
				...
				'leanroles_bulk_tag'   => 'gold',
				'users'                => array( $this->user_id ),
				'leanroles_bulk_nonce' => 'obviously-wrong',
			)
		);
	...
	public function test_the_nothing_notice_is_a_warning(): void {
		$_GET = array( 'leanroles_bulk' => 'nothing' );
