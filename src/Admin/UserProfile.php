...
final class UserProfile {
	...
	private const NONCE = 'leanroles_profile_tags';
	...
	public static function render( $user ): void {
		...
		$editable = current_user_can( 'promote_users' ) && current_user_can( 'edit_user', $user->ID );
		...
		if ( $editable ) {
			wp_nonce_field( self::NONCE, 'leanroles_profile_nonce', false );
			echo '<input type="hidden" name="leanroles_tags_submitted" value="1" />';
		}
		...
		foreach ( $terms as $term ) {
			printf(
				'<label class="leanroles-checkbox"><input type="checkbox" name="leanroles_tags[]" value="%s" %s %s /> %s</label><br />',
				esc_attr( $term->slug ),
				checked( isset( $current[ $term->slug ] ), true, false ),
				disabled( $editable, false, false ),
				TagsPage::badge( $term->name, (string) get_term_meta( $term->term_id, Taxonomy::META_COLOR, true ) ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			);
		}
	...
	public static function save( $user_id ): void {
		if ( empty( $_POST['leanroles_tags_submitted'] ) ) {
			return;
		}
		...
		check_admin_referer( self::NONCE, 'leanroles_profile_nonce' );

		$submitted = isset( $_POST['leanroles_tags'] ) ? (array) wp_unslash( $_POST['leanroles_tags'] ) : array();
		$slugs     = array();
