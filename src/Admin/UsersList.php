...
final class UsersList {
	...
	private const NONCE = 'leanroles_bulk_tags';
	...
	public static function bulk_notice(): void {
		...
		if ( ! isset( $_GET['leanroles_bulk'] ) ) {
			return;
		}

		$outcome = sanitize_key( wp_unslash( $_GET['leanroles_bulk'] ) );
		$count   = isset( $_GET['leanroles_count'] ) ? (int) $_GET['leanroles_count'] : 0;
	...
	public static function add_column( array $columns ): array {
		...
		foreach ( $columns as $key => $label ) {
			...
			if ( 'role' === $key ) {
				$out['leanroles_tags'] = __( 'Tags', 'leanroles' );
			}
		}

		if ( ! isset( $out['leanroles_tags'] ) ) {
			$out['leanroles_tags'] = __( 'Tags', 'leanroles' );
		}
	...
	public static function render_column( $output, $column, $user_id ) {
		...
		foreach ( $tags as $slug ) {
			$html .= sprintf(
				...
				TagsPage::badge( $catalogue[ $slug ]['name'] ?? $slug, $catalogue[ $slug ]['color'] ?? '' )
			);
		}
	...
	public static function add_views( array $views ): array {
		...
		foreach ( $terms as $term ) {
			...
			$views[ 'leanroles_' . $term->slug ] = sprintf(
	...
	public static function render_controls( $which ): void {
		if ( 'top' !== $which || ! current_user_can( 'promote_users' ) ) {
			return;
		}
		...
		wp_nonce_field( self::NONCE, 'leanroles_bulk_nonce', false );

		echo '<label class="screen-reader-text" for="leanroles-bulk-tag">' . esc_html__( 'Tag', 'leanroles' ) . '</label>';
		echo '<select name="leanroles_bulk_tag" id="leanroles-bulk-tag">';
		...
		submit_button( __( 'Add tag', 'leanroles' ), '', 'leanroles_bulk_add', false );
		submit_button( __( 'Remove tag', 'leanroles' ), '', 'leanroles_bulk_remove', false );
	}
	...
	public static function handle_bulk(): void {
		if ( empty( $_REQUEST['leanroles_bulk_add'] ) && empty( $_REQUEST['leanroles_bulk_remove'] ) ) {
			return;
		}
		...
		check_admin_referer( self::NONCE, 'leanroles_bulk_nonce' );

		$slug = isset( $_REQUEST['leanroles_bulk_tag'] ) ? sanitize_key( wp_unslash( $_REQUEST['leanroles_bulk_tag'] ) ) : '';
		$ids  = isset( $_REQUEST['users'] ) ? array_map( 'intval', (array) wp_unslash( $_REQUEST['users'] ) ) : array();

		if ( '' === $slug || ! Catalogue::has( $slug ) || ! $ids ) {
			wp_safe_redirect( add_query_arg( 'leanroles_bulk', 'nothing', admin_url( 'users.php' ) ) );
			exit;
		}

		$remove = ! empty( $_REQUEST['leanroles_bulk_remove'] );
		$count  = 0;
		...
		wp_safe_redirect(
			add_query_arg(
				array(
					'leanroles_bulk'  => $remove ? 'removed' : 'added',
					'leanroles_count' => $count,
					'user_tag'        => $slug,
				),
				admin_url( 'users.php' )
			)
