...
final class Csv {
	...
	public static function import_assignments( array $rows, bool $create_tags = false, bool $replace = false ): array {
		...
		if ( ! isset( $index['tags'] ) ) {
			$result['errors'][] = __( 'The file has no "tags" column.', 'leanroles' );
			return $result;
		}

		if ( ! isset( $index['user_id'] ) && ! isset( $index['user_login'] ) && ! isset( $index['user_email'] ) ) {
			$result['errors'][] = __( 'The file needs a user_id, user_login or user_email column.', 'leanroles' );
			return $result;
		}
		...
		foreach ( $rows as $line => $row ) {
			...
			if ( ! $user ) {
				...
				$result['errors'][] = sprintf(
					...
					__( 'Line %d: no matching user.', 'leanroles' ),
					$line + 2
				);
				continue;
			}
			...
			foreach ( $slugs as $slug ) {
				...
				if ( ! $create_tags ) {
					$result['errors'][] = sprintf(
						...
						__( 'Line %1$d: tag "%2$s" does not exist.', 'leanroles' ),
						$line + 2,
						$slug
					);
					continue;
